...
passport.use('facebook', new BearerStrategy((token, done) => {
  facebookService.getUser(token).then((user) => {
    return User.createFromService({ service: 'facebook', ...user })
  }).then((user) => {
    done(null, user)
    return null
  }).catch(done)
...
passport.use('github', new BearerStrategy((token, done) => {
  githubService.getUser(token).then((user) => {
    return User.createFromService({ service: 'github', ...user })
  }).then((user) => {
    done(null, user)
    return null
  }).catch(done)
...
passport.use('google', new BearerStrategy((token, done) => {
  googleService.getUser(token).then((user) => {
    return User.createFromService({ service: 'google', ...user })
  }).then((user) => {
    done(null, user)
    return null
  }).catch(done)
...
passport.use('apple', new BearerStrategy((token, done) => {
  appleService.getUser(token).then((user) => {
    return User.createFromService({ service: 'apple', ...user })
  }).then((user) => {
    done(null, user)
    return null
  }).catch(done)
...
passport.use('vk', new BearerStrategy((token, done) => {
  vkService.getUser(token).then((user) => {
    return User.createFromService({ service: 'vk', ...user })
  }).then((user) => {
    done(null, user)
    return null
  }).catch(done)
